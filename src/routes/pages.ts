...
pagesRouter.get("/", async (req: AuthedRequest, res) => {
  const [rows] = await pool.query(
    "SELECT id, user_id AS userId, title, blocks, published, created_at AS createdAt, updated_at AS updatedAt FROM pages WHERE user_id = ? ORDER BY updated_at DESC",
    [req.userId]
  );
  res.json(rows);
});
...
pagesRouter.get("/:id", async (req: AuthedRequest, res) => {
  const [rows]: any = await pool.query(
    "SELECT id, user_id AS userId, title, blocks, published, created_at AS createdAt, updated_at AS updatedAt FROM pages WHERE id = ? AND user_id = ?",
